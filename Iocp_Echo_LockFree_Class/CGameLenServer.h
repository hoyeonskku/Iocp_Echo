...
public:
	...
	virtual void	OnAccept(/*Client 정보 / SessionID / 기타등등*/) override ; // < Accept 후 접속처리 완료 후 호출.
	virtual void 	OnRelease(unsigned long long sessionID) override; // < Release 후 호출
	virtual void 	OnRecv(unsigned long long sessionID, CPacket* pPacket) ; // < 패킷 수신 완료 후
